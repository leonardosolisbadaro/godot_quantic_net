...
func test_integracao_dois_clientes_sob_netem_pesado() -> void:
	...
	while elapsed < 3000:
		if (
			cli1.get_state() == srv.ConnectionState.CONNECTED
			and cli2.get_state() == srv.ConnectionState.CONNECTED
		):
			break
	...
	if cli1_ver_c2 != null and not cli1_ver_c2.is_empty():
		...
		assert_true(
			dist_cli1_c2 < 2.0,
			"cli1 ve a posicao de c2 convergir remotamente (dist=" + str(dist_cli1_c2) + ")",
		)
	...
	assert_true(
		loss1 >= 0 and loss1 <= 0.35,
		"perda medida de cli1 no host deve ficar <= 35% (" + str(loss1) + ")",
	)
	assert_true(
		loss2 >= 0 and loss2 <= 0.35,
		"perda medida de cli2 no host deve ficar <= 35% (" + str(loss2) + ")",
	)
...
func test_isolamento_netem_entre_dois_clientes() -> void:
	...
	while elapsed < 3000:
		if (
			cli_lat.get_state() == srv.ConnectionState.CONNECTED
			and cli_fast.get_state() == srv.ConnectionState.CONNECTED
		):
			break
		await wait_process_frames(1, "waiting handshake")
		elapsed += 16
