...
func test_excesso_moderado_gera_clamp() -> void:
	...
	assert_almost_eq(r["pos"].x, 0.3, 0.001, "O servidor deve cortar o vetor excedente pelo limite do MAX_SPEED")
...
func test_fora_do_mundo_rejeitado() -> void:
	...
	assert_eq(r["action"], "reject", "Posicao out-of-bounds tem que gerar reject independentemente da velocidade")
...
func test_strikes_acumulam_ate_kick() -> void:
	...
	assert_true(v.should_kick(player_id), "Estourar o MAX_STRIKES de rejeições deve acionar o should_kick")
