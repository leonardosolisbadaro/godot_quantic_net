...
func test_drain_remove_confirmados_e_retorna_pendentes() -> void:
	...
	assert_eq(
		replay[0]["seq"],
		3,
		"O primeiro pendente obrigatoriamente e o logo apos o confirmado",
	)
...
func test_drain_com_wrap_around_limpa_corretamente() -> void:
	...
	assert_eq(
		replay.size(),
		1,
		"Wrap-around (65535 -> 0) deve ser tolerado matematicamente na comparacao",
	)
	assert_eq(
		replay[0]["seq"],
		1,
		"Somente a sequence 1 eh estritamente posterior a 0 no wrap-around",
	)
