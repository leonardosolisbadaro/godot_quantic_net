...
func test_converge_para_offset_real_com_jitter() -> void:
	...
	assert_almost_eq(
		...
		real_offset,
		30.0,
		"O offset estimado com filtro EMA deve absorver grande parte do jitter da rede",
	)
...
func test_descarta_rtt_absurdo() -> void:
	...
	assert_false(
		cs.is_synced(),
		"Um RTT gigante (>5000ms) deve ser completamente ignorado para proteger a integridade do offset",
	)
...
func test_server_time_aplica_offset() -> void:
	...
	assert_eq(
		cs.server_time(),
		int(Time.get_ticks_msec() + cs.offset_ms),
		"O tempo calculado do servidor deve ser o uptime local mais a defasagem (offset)",
	)
...
func test_primeiro_pong_inicializa_sem_ema() -> void:
	...
	assert_eq(
		cs.rtt_ms,
		80.0,
		"O primeiro RTT da conexao deve ser adotado diretamente sem suavizacao EMA",
	)
...
func test_calcula_jitter_baseado_na_variancia_do_rtt() -> void:
	...
	assert_almost_eq(
		...
		20.0,
		0.1,
		"O Jitter deve refletir a variancia do RTT utilizando EMA",
	)
