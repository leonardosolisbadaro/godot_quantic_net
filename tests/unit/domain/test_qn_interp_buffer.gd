...
func test_pacote_atrasado_descartado() -> void:
	...
	assert_eq(buf.get_snaps_size(), 2, "Pacotes com timestamp <= ao ultimo cabecalho registrado devem ser dropados sumariamente")
...
func test_dynamic_jitter_expands_delay() -> void:
	...
	assert_eq(buf.get_target_delay_ms(), 220.0, "O delay deve dilatar proporcionalmente ao Jitter recebido")
	
	# Act 2 (Ação): Jitter catastrofico de 500ms
	buf.update_jitter(500.0)
	
	# Assert 2 (Verificação): Não pode dilatar infinitamente
	assert_eq(buf.get_target_delay_ms(), 250.0, "O delay nao pode ultrapassar o teto maximo de seguranca de 250ms")
...
func test_error_blending_amortece_pulo_visual() -> void:
	...
	assert_true(interp_s.get("pos", Vector3.ZERO).x < 0.225, "Error Blending deve decair o erro antigo")
