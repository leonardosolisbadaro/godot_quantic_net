...
func test_sem_gaps_reporta_zero_perda() -> void:
	...
	assert_eq(t.received, 50, "Todos os pacotes sequenciais devem ser contabilizados como recebidos")
...
func test_pacote_duplicado_ou_atrasado_ignorado() -> void:
	...
	assert_eq(t.received, 1, "Pacotes duplicados e atrasados nao contabilizam como novos recebimentos")
...
func test_wrap_16bit_nao_conta_perda_falsa() -> void:
	...
	assert_eq(t.lost, 0, "O wrap-around de 16-bits (65535 -> 0) nao pode ser tratado como perda ou atraso")
...
func test_loss_pct_reflete_janela_recente() -> void:
	...
	assert_almost_eq(pct, 60.0, 0.1, "3 pacotes perdidos em um total de 5 pacotes na janela significa 60% de perda")
