...
func test_roundtrip_preserva_valores_dentro_da_precisao() -> void:
	...
	var b: PackedByteArray = QNSerializer.encode_state_seq(seq, pos, rot, expected_ts, expected_custom_id)
...
func test_seq_faz_wrap_em_16_bits() -> void:
	...
	var b: PackedByteArray = QNSerializer.encode_state_seq(high_seq, Vector3.ZERO, Vector3.ZERO, 0, 0)
	var d: Dictionary = QNSerializer.decode_state_seq(b)
...
func test_snapback_usa_mesmo_formato_com_reason_no_custom_id() -> void:
	...
	var b: PackedByteArray = QNSerializer.encode_snapback(9, Vector3.ONE, Vector3.ZERO, 42, expected_reason)
	var d: Dictionary = QNSerializer.decode_state_seq(b)
	
	# Assert (Verificação): O reason de snapback transita exatamente no mesmo campo "custom_id"
	assert_eq(d.get("custom_id", 0), expected_reason, "A razao do snapback deve viajar acoplada no campo custom_id")
